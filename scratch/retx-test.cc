...
main(int argc, char* argv[])
{
    ...
    topology->switches_begin()->nodePtr->GetDevice(1)->SetAttribute("ReceiveErrorModel",
                                                                    PointerValue(em));
    topology->switches_begin()->nodePtr->GetDevice(2)->SetAttribute("ReceiveErrorModel",
                                                                    PointerValue(em));
    ...
    std::cout << "Total used time: " << elapsed_seconds.count() << "s" << std::endl;
    Simulator::Destroy();
}
